...
accprio_b1s = np.logspace(0, np.log(10), 12, base=np.e) # np.linspace(1, 30, 12)
...
def run_polynomial_cost_exp(mu1, mu2, c1, c2, d1, p1=0.5):
    ...
    exp_name = 'polynomial_cost_exp4' #3
...
def run_polynomial_constant_exp(mu1, mu2, c1_deg, c2, p1=0.5):
    exp_name = 'polynomial_const_exp'
    C1_fn, C2_fn = lambda t : t**(c1_deg+1)/(c1_deg+1), lambda t:c2 * t

    # (alpha_star + Exp(mu1-l1))**deg = c2
...
if __name__ == "__main__":
    ...
    #plot.gen_plot('polynomial_cost_exp2', None, p1 = 0.75)
