...
def run_2Class_MG1_tests(test_label, l1, l2, S1, S2, policy):
    ...
    T1, T2 = theoretical_MG1.T(policy.policy_name)
    E1, ET1, ET1sq = T1
    E1, ET2, ET2sq = T2
...
def run_NPPrio_tests(test_label, l1, l2, S1, S2, policy):
    simulated_MG1 = simulations.MG1([simulations.JobClass(1, l1, S1),
...
if __name__ == "__main__":
    ...
    run_2Class_MG1_tests("2cMH1c", l1, l2, lib.hyperexponential(mu1, Csq=10),
                         lib.hyperexponential(mu2, Csq=10), policy.PPrio12)    
    
    print("**2 CLASS NP-ACC-PRIO TESTS**")
    b1, b2 = 100, 1 # should be close to NPPrio12
    run_2Class_MG1_tests("2cMM1acc100", l1, l2, lib.exp(mu1), lib.exp(mu2), policy.AccPrio(b1, b2))
